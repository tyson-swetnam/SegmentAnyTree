...
ENV CU_VERSION=cu124
ENV TORCH_CUDA_ARCH_LIST="7.0;7.5;8.0;8.6;8.9;9.0;10.0"
...
RUN python3 -m pip install --no-cache-dir \
    ...
    torch-spline-conv \
    torch-geometric \
    -f https://data.pyg.org/whl/torch-2.5.1+${CU_VERSION}.html

# ---- SpConv v2.x (pip install, no source build needed!) ----
RUN python3 -m pip install --no-cache-dir spconv-cu124
...
# Fix stale GPG keys in NVIDIA base image, then install runtime packages
RUN rm -rf /var/lib/apt/lists/* && \
    apt-key del 7fa2af80 2>/dev/null; \
    apt-get clean && \
...
COPY docker/jupyter_notebook_config.json /home/sat/.jupyter/jupyter_notebook_config.json

# Verify model weights are real files (not Git LFS pointers)
# If PointGroup-PAPER.pt is <1KB, it's an LFS pointer and the build should fail.
RUN MODEL_SIZE=$(stat -c%s "${SAT_ROOT}/model_file/PointGroup-PAPER.pt" 2>/dev/null || echo 0) && \
    if [ "$MODEL_SIZE" -lt 1000 ]; then \
      echo "ERROR: model_file/PointGroup-PAPER.pt is a Git LFS pointer (${MODEL_SIZE} bytes)." && \
      echo "Run 'git lfs pull --include=model_file/PointGroup-PAPER.pt' before building." && \
      exit 1; \
    fi && \
    echo "Model weights verified: ${MODEL_SIZE} bytes"

# Migrate weights to SpConv format
RUN cd ${SAT_ROOT} && \
